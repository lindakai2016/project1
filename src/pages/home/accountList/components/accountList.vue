        ...
        <div class="tbbody">
            <div class="acItem" v-for="(item, i) in accountList" :key="i">
                ...
                <div class="td col_cz">
                    ...
                    <i class="iconfont icon-shanchu link czIc hov" title="删除" @click="deleteAc(item)"></i>
                </div>
            </div>
            <div class="empty" v-if="!hasAccount">
                <img src="@/static/images/home_img_empty.svg">
                <p class="emTxt">无搜索结果</p>
            </div>
        </div>
         <!--账号详情-->
        <basePopup v-model="showAcDt">
            <accountDetail @exit="exitDetail" :acItem="acItem"></accountDetail>
        </basePopup>
        <!--编辑账号-->
        <basePopup v-model="showAcEd">
            <newAccount @exit="exitEdit" @submitOk="submitEdit" :acItem="acItem" :type="1"></newAccount>
        </basePopup>
    </div>
</template>
