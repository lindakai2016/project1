<template>
    ...
    <div class="cityCarType bookForm">
        ...
        <p class="fi">
            ...
            <poiInput class="rval v_sPos" :err="poiItemErr" :cityCode="sCtCode" :item="poiItem" @change="poiChange" @changeCity="poiCityChange" @blur="poiItemBlur"></poiInput>
        </p>
        <p class="fi">
            <label class="lb needed">目的地</label>
            <poiInput class="rval v_ePos" :err="poiItem2Err" :cityCode="eCtCode" :item="poiItem2" @change="poiChange2" :cityId="cityId" @blur="poiItem2Blur"></poiInput>
        </p>
        <p class="fi">
...
export default {
    ...
    computed: {
        formErr() {
            return this.poiItemErr || this.poiItem2Err || this.jcDateErr || this.jcTimeErr || this.rideNumErr || this.userPhoneErr || "";
        },
        canSubmit() {
            let bol = this.rideNum && this.rideNum.validRideNum() && this.userPhone && this.userPhone.validPhone() && this.jcDate && this.jcTime;
            bol = bol && this.poiItem && this.poiItem.name && this.poiItem2 && this.poiItem2.name;
            return bol;
        }
    },
    watch: {
    ...
    methods: {
        ...
        poiChange2(poiItem) {
            this.poiItem2 = poiItem;
            this.updateMap();
        },
        // 更新地图
        updateMap() {
            if(this.poiItem.name && this.poiItem2.name) {
                let sPos = this.poiItem.location || {};
                let ePos = this.poiItem2.location || {};
                this.drawMapLine(sPos.lng, sPos.lat, ePos.lng, ePos.lat);
            }
        },
        ...
        poiItem2Blur() {
            if(!this.poiItem2 || !this.poiItem2.name) {
                this.poiItem2Err = "目的地不能为空";
                return;
            }
            this.poiItem2Err = "";
            return;
